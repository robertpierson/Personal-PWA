...
- **Primary color is a vivid royal/cobalt blue** (`--color-forest: #2451d6`, deep variant `--color-forest-deep: #16309e` — token name kept from the earlier bottle-green iteration since dozens of `bg-forest`/`text-forest`/`border-forest` classes reference it, and renaming the token buys no visual benefit). Blue + gold was chosen as the explicit brand direction after the initial "civic ledger" bottle-green palette shipped; the civic/municipal reasoning still holds (blue is the color of seals, ribbons, and official letterhead, same register as the green it replaced). The first blue pass (`#1b3a6b`) read as a desaturated near-navy; it was brightened and re-saturated per explicit follow-up feedback so the primary reads as a distinct, colorful blue next to the gold rather than a muted dark neutral. Contrast re-verified: `#2451d6` on `--color-paper` (`#f2f0e8`) ≈5.9:1 (passes AA for body text, not just large text/UI).
- **Background is cooler/grayer stone** (`--color-paper: #f7f6f1`, cards on `--color-panel: #ffffff`), not golden cream. Brightened from the original `#f2f0e8`/`#fbfaf5` pass per explicit follow-up feedback — the warm parchment tone was muddying both the blue and the gold; near-white cards give both accent colors more contrast to pop against. `--color-paper-2` (section-band backgrounds) was deepened to `#e3dfcf` for the same reason — alternating bands now read as more distinct.
- **Gold given more presence**, also per explicit follow-up feedback: every `.eyebrow` label sitewide (28 instances) now carries a short gold tick mark before it; the Presence Scorecard's checklist icons and the homepage/services icon circles switched from blue to gold; the proof-section stat numbers switched from blue to `--color-gold-text`. Gold is still never used as a large background wash or small body text — only icons, ticks, and badges, where its contrast is adequate (see below).
...
- **Contrast re-verified by hand** against the brightened background: ink/paper ≈16.5:1, forest/paper ≈6.1:1, gold-text/panel (white) ≈7.7:1, ink/gold (scorecard check icons) ≈5.5:1. Gold itself (`#b8862e`) still measures only ≈2.6:1 on paper — still confirmed decorative-only (icons, ticks, borders, badges with dark text on top), never used as small body/label text on a light background. One real bug was caught and fixed during the initial build: a testimonial label combined `.eyebrow-gold` (light gold, meant for dark backgrounds) with a light card background — see §12 in the final report.
- **Blue expanded into a tonal ramp** (`--color-forest-100: #dce6ff`, `-300: #5b82f0`, base `#2451d6`, `-deep: #16309e`, `-ink: #0a1233`), per an explicit follow-up request to make the site "more vibrant" and closer to a bold modern-agency aesthetic (referencing a competitor's digital-transformation landing page). This is a deliberate, explicit reversal of two earlier decisions: the original "one flat primary, restrained everywhere" color rule, and the removal of a `HeroAurora` glow blob during the initial audit (flagged then as an "AI-site tell" — see §1). Both reversals are intentional per direct user request, not drift. The tonal ramp is still blue-only — no new hue family was introduced, so the "one primary family, one accent" spirit of the original rule holds even though it's no longer a single flat color.
- **One dark full-bleed band** (`bg-gradient-to-br from-forest-ink via-forest-deep to-forest-ink`, used once on the homepage's "one system" section) plus blurred `blur-3xl` glow orbs (blue and gold, `pointer-events-none`/`aria-hidden`, positioned off-canvas so they never overlap text) on the hero, both CTA bands, and the highlighted pricing tile. Kept to a small, deliberate set of high-impact spots rather than applied sitewide, so the rest of the site stays the light civic-ledger surface. Text on the dark band uses `--color-paper` at full or reduced opacity — contrast checked at ≈17:1 (full) and ≈13:1 (`/75` and `/70` opacity) against `--color-forest-ink`, both comfortably above AA.
- **Icon chips alternate gold/blue** (`bg-gold/15` / `bg-forest-100`) on the services grids (homepage and `/services`), instead of a single flat color, for more color variety per the same follow-up request.
- **The last warm tones were removed** per explicit follow-up feedback ("get rid of the cream color"): `--color-paper` (`#f7f6f1`→`#f6f7fa`), `--color-paper-2` (`#e3dfcf`→`#e7ebf3`), `--color-line`/`-strong`, and `--color-ink-soft`/`-faint` (which still carried a greenish tint left over from the original bottle-green era) all moved to true cool white/blue-gray. There is no warm hue anywhere in the palette now — white, cool gray, blue, gold, that's the whole system.
- **A dark theme was added for the dashboard app-shell only** (not the marketing site, which stays light-only per the brand decision at the top of this doc): a `[data-theme="dark"]` block in `globals.css` overrides the same custom properties the whole dashboard already reads (`--color-paper`, `-ink`, `-line`, the forest/gold families), scoped to the dashboard's root wrapper (`data-app-theme` in `(app)/dashboard/layout.tsx`) rather than `:root`. Because no dashboard component hard-codes a color — confirmed by grep, zero hex/rgb/`bg-white`/`text-black` literals in `src/app/(app)` or `src/components/dashboard` — the whole dashboard re-themes with zero component changes. Toggle lives in the new Settings > Appearance panel (`components/dashboard/theme-toggle.tsx`), persisted to `localStorage`. Applied via `components/dashboard/theme-init.tsx`, a client component using `useLayoutEffect` (fires before paint, so no visible flash) — not a raw `<script>` tag, which React 19 flags as unreliable when rendered as a JSX child (its content isn't guaranteed to (re-)execute outside the initial HTML parse; that was the first implementation and was replaced after it surfaced a console error in dev). Dark contrast verified: ink/paper ≈19:1, forest/paper ≈5.2:1, gold-soft/paper ≈14:1.
- **Settings page opened to all roles**, not just owners — theme and notification preferences are personal/per-device, so gating the whole page to owners (as the original Organization/Connected-accounts/Team-access panels required) no longer made sense. The page now shows Appearance, Notifications, and Account to everyone, and conditionally renders the three business-sensitive panels only for owners.
